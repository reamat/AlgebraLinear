...
\boxed{
	\begin{split}
	\text{Todo vetor pode ser representado de maneira única \, } \\
	\text{como combinação linear dos elementos de uma base.}
	\end{split}
}
\]
...
\boxed{
	\begin{split}
	\text{Toda base de um espaço vetorial fixado } \\
	\text{possui o mesmo número de elementos.}
	\end{split}
}
\]
...
\boxed{
	\begin{split}
	\text{Uma base para $\Nul A$ pode ser encontrada por escalonamento ao resol- } \\
	\text{ver o sistema homogêneo associado à matriz $A$. Além disso, a dimensão} \\
	\text{do espaço nulo é igual ao número de variáveis livres do sistema.} \qquad
	\end{split}
}
\]
...
\boxed{
	\begin{split}
	\text{Uma base para $\Col A$ é formada pelas colunas pivô da matriz $A$. Para descobrir quais são }\ \ \\
	\text{as colunas pivô, procedemos por escalonamento. As \textit{colunas da matriz original} formam uma} \\
	\text{base de $\Col A$. A dimensão do espaço coluna é igual ao número de colunas pivô da matriz. }
	\end{split}
}
\]
