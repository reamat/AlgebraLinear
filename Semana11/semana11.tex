...
y_1 \\ y_2 \\ \vdots \\ y_n
\end{bmatrix}
\end{equation} vetores de $\bR^n$ representados na base canônica, e definimos o \textbf{produto escalar} ou \textbf{produto interno} entre $\vec{x}$ e $\vec{y}$:
\begin{equation}
...
\begin{example}\label{canon}
	...
	1/4 \\ 1/4 \\ 1/4 \\ -1/4
	\end{bmatrix}.
	\end{equation} A projeção de $\vec{v}$ sobre o vetor $\vec{e}_3$ da base canônica de $\bR^4$ é (observe que $\|\vec{e}_3\| = 1$)
	\begin{equation}
...
\end{equation} O teorema acima afirma, em outras palavras, quando a base é ortogonal, os coeficientes do vetor $\vec{v}$ na base $\cB$ são os números
\begin{equation}
...
\begin{example}
	...
	-1 \\ 2
	\end{bmatrix}
	\end{equation} formam uma base ortogonal para $\bR^2$.
	...
	\end{equation} Segue que $\vec{u}$ e $\vec{v}$ são também ortogonais e, logo, formam uma base ortogonal de $\bR^2$.
...
\end{equation} Uma consequência imediata é que, sendo $\{\vec{v}_1, \vec{v}_2, \cdots, \vec{v}_n\}$ uma base ortonormal de $\bR^n$, podemos escrever
\begin{equation}
...
\begin{example}
	...
	0 \\ \sqrt{2}/2 \\ - \sqrt{2}/2
	\end{bmatrix}
	\end{equation} formam uma base ortonormal de $\bR^3$: verifique! Qualquer vetor $\vec{x} \in \bR^3$ pode ser escrito como 
	\begin{equation}
...
\begin{example}
	...
	1 \\ -2 \\ 1
	\end{bmatrix}
	\end{equation} formam uma base ortogonal de $\bR^3$. Normalizando cada um dos vetores, podemos obter uma base ortonormal de $\bR^3$:
	\begin{equation}
	\left\{
	\begin{array}{ll}
	...
	\end{bmatrix}
	\end{equation} formam um base ortonormal de $\bR^3. \ \lhd$
\end{example}
